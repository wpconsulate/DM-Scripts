...
jQuery(document).ready( function($) {
	...
	$("#ui-datepicker-div").wrap('<div class="dm_element" />');

	/**
	 * Initialize color picker
	 */
	if (typeof jQuery.wp === 'object' && typeof jQuery.wp.wpColorPicker === 'function') {
		$('input:text.dm_colorpicker').wpColorPicker();
	} else {
		$('input:text.dm_colorpicker').each( function(i) {
	...
	$('.dm_metabox')
	.on( 'change', '.dm_upload_file', function() {
		formfield = $(this).attr('name');
		$('#' + formfield + '_id').val("");
	})
	.on( 'click', '.dm_upload_button', function(event) {
		...
		var attachment = true;
		var isList = $self.hasClass( 'dm_upload_list' );
		...
		file_frames[formfield] = wp.media.frames.file_frame = wp.media({
			...
			button: {
				text: window.dm_l10.upload_file
			},
		...
		file_frames[formfield].on( 'select', function() {
			...
			if ( isList ) {
				...
				$( attachment ).each( function() {
					if ( this.type && this.type === 'image' ) {
						...
						uploadStatus = '<li class="img_status">'+
							...
							'<p><a href="#" class="dm_remove_file_button" rel="'+ formfield +'['+ this.id +']">'+ dm_l10.remove_image +'</a></p>'+
							'<input type="hidden" id="filelist-'+ this.id +'" name="'+ formfield +'['+ this.id +']" value="' + this.url + '">'+
						'</li>';

					} else {
						// Standard generic output if it's not an image.
						uploadStatus = '<li>'+ dm_l10.file +' <strong>'+ this.filename +'</strong>&nbsp;&nbsp;&nbsp; (<a href="' + this.url + '" target="_blank" rel="external">'+ dm_l10.download +'</a> / <a href="#" class="dm_remove_file_button" rel="'+ formfield +'['+ this.id +']">'+ dm_l10.remove_file +'</a>)'+
							'<input type="hidden" id="filelist-'+ this.id +'" name="'+ formfield +'['+ this.id +']" value="' + this.url + '">'+
						'</li>';
				...
				// Append each item from our fileGroup array to .dm_media_status
				$( fileGroup ).each( function() {
					$formfield.siblings('.dm_media_status').slideDown().append(this);
				});
				...
				if ( attachment.type && attachment.type === 'image' ) {
					// image preview
					uploadStatus = '<div class="img_status"><img style="max-width: 350px; width: 100%; height: auto;" src="' + attachment.url + '" alt="'+ attachment.filename +'" title="'+ attachment.filename +'" /><p><a href="#" class="dm_remove_file_button" rel="' + formfield + '">'+ dm_l10.remove_image +'</a></p></div>';
				} else {
					// Standard generic output if it's not an image.
					uploadStatus = dm_l10.file +' <strong>'+ attachment.filename +'</strong>&nbsp;&nbsp;&nbsp; (<a href="'+ attachment.url +'" target="_blank" rel="external">'+ dm_l10.download +'</a> / <a href="#" class="dm_remove_file_button" rel="'+ formfield +'">'+ dm_l10.remove_file +'</a>)';
				}

				// add/display our output
				$formfield.siblings('.dm_media_status').slideDown().html(uploadStatus);
			}
		});
	...
	.on( 'click', '.dm_remove_file_button', function(event) {
		var $self        = $(this);
		if ( $self.is( '.attach_list .dm_remove_file_button' ) ){
		...
		if ( ! $container.length ) {
			$self.parents('.dm_media_status').html('');
		} else {
			$container.html('');
		}
		return false;
	...
	.on( 'click', '.add-row-button', function(e) {
		...
		var input = $('input.dm_datepicker',row);
	...
	// ajax when typing
	.on( 'keyup', '.dm_oembed', function(event) {
		// fire our ajax function
		doDMajax($(this), event);
	});

	// ajax on paste
	$('.dm_oembed').bind( 'paste', function(e) {
		var pasteitem = $(this);
		// paste event is fired before the value is filled, so wait a bit
		setTimeout( function() {
			// fire our ajax function
			doDMajax(pasteitem, 'paste');
		}, 100);
	}).blur( function() {
		// when leaving the input
		setTimeout( function() {
			// if it's been 2 seconds, hide our spinner
			$('.postbox table.dm_metabox .dm-spinner').hide();
		}, 2000);
	});

	// function for running our ajax
	function doDMajax(obj, e) {
		// get typed value
		...
		if (e === 'paste' || e.which <= 90 && e.which >= 48 || e.which >= 96 && e.which <= 111 || e.which == 8 || e.which == 9 || e.which == 187 || e.which == 190) {
			...
			var context = obj.parents('.dm_metabox tr td');
			...
			// show our spinner
			$('.dm-spinner', context).show();
			// clear out previous results
			$('.embed_wrap', context).html('');
			// and run our ajax function
			setTimeout( function() {
				// if they haven't typed in 500 ms
				if ($('.dm_oembed:focus').val() != oembed_url)
					return;
				$.ajax({
					type : 'post',
					dataType : 'json',
					url : window.dm_l10.ajaxurl,
					data : {
						'action': 'dm_oembed_handler',
						'oembed_url': oembed_url,
						'oembed_width': oembed_width > 300 ? oembed_width : 300,
						'field_id': field_id,
						'object_id': window.dm_l10.object_id,
						'object_type': window.dm_l10.object_type,
						'dm_ajax_nonce': window.dm_l10.ajax_nonce
					},
	...
	function resizeoEmbeds() {
		$('table.dm_metabox').each( function( index ) {
			var self = $(this);
			...
			var child_el = $('.dm-type-oembed .embed_status', self).children().first();
