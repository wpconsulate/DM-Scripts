...
class dm_fields_types {
	 ...
	 * @since  1.0.0
	 * @return dm_fields_types A single instance of this class.
	 */
	...
	private static function desc( $desc, $paragraph = false ) {
		...
		return "\n<$tag class=\"dm_metabox_description\">$desc</$tag>\n";
	}
	...
	private static function repeat_table_open( $class = '' ) {
		echo self::desc( self::$field['desc'] ), '<table id="', self::$field['id'], '_repeat" class="dm-repeat-table ', $class ,'"><tbody>';
	}
	...
	private static function repeat_table_close() {
		echo '</tbody></table><p class="add-row"><a data-selector="', self::$field['id'] ,'_repeat" class="add-row-button button" href="#">'. __( 'Add Row', 'dm' ) .'</a></p>';
	}
	...
	private static function repeat_cell( $input ) {
		return '<td>'. $input .'</td><td class="remove-row"><a class="button remove-row-button" href="#">'. __( 'Remove', 'dm' ) .'</a></td>';
	}
	...
	public static function is_valid_img_ext( $file ) {
		...
		self::$valid = empty( self::$valid ) ? (array) apply_filters( 'dm_valid_img_types', array( 'jpg', 'jpeg', 'png', 'gif', 'ico', 'icon' ) ) : self::$valid;
	...
	public static function text_small( $field, $meta ) {
		echo '<input class="dm_text_small" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function text_small_repeat( $field, $meta ) {
		self::repeat_text_field( $field, $meta, 'dm_text_small' );
	}

	public static function text_medium( $field, $meta ) {
		echo '<input class="dm_text_medium" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function text_medium_repeat( $field, $meta ) {
		self::repeat_text_field( $field, $meta, 'dm_text_medium' );
	}

	public static function text_email( $field, $meta ) {
		echo '<input class="dm_text_email dm_text_medium" type="email" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'], true );
	}

	public static function text_email_repeat( $field, $meta ) {
		self::repeat_text_field( $field, $meta, 'dm_text_email dm_text_medium', 'email' );
	}
	...
	public static function text_url( $field, $meta ) {
		...
		echo '<input class="dm_text_url dm_text_medium" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', $val, '" />', self::desc( $field['desc'], true );
	}
	...
	public static function text_url_repeat( $field, $meta ) {
		...
		$protocols = isset( $field['protocols'] ) ? (array) $field['protocols'] : null;
		$val = $val ? esc_url( $val, $protocols ) : '';
		self::repeat_text_field( $field, $val, 'dm_text_url dm_text_medium' );
	}

	public static function text_date( $field, $meta ) {
		echo '<input class="dm_text_small dm_datepicker" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function text_date_timestamp( $field, $meta ) {
		echo '<input class="dm_text_small dm_datepicker" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? date( 'm\/d\/Y', $meta ) : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function text_datetime_timestamp( $field, $meta, $object_id ) {

		// This will be used if there is a select_timezone set for this field
		$tz_offset = dm_fields::field_timezone_offset( $object_id );
		if ( !empty( $tz_offset ) ) {
			$meta -= $tz_offset;
		}

		echo '<input class="dm_text_small dm_datepicker" type="text" name="', $field['id'], '[date]" id="', $field['id'], '_date" value="', '' !== $meta ? date( 'm\/d\/Y', $meta ) : $field['std'], '" />';
		echo '<input class="dm_timepicker text_time" type="text" name="', $field['id'], '[time]" id="', $field['id'], '_time" value="', '' !== $meta ? date( 'h:i A', $meta ) : $field['std'], '" />', self::desc( $field['desc'] );
	}
	...
	public static function text_datetime_timestamp_timezone( $field, $meta ) {
		...
		echo '<input class="dm_text_small dm_datepicker" type="text" name="', $field['id'], '[date]" id="', $field['id'], '_date" value="', '' !== $meta ? date( 'm\/d\/Y', $meta ) : $field['std'], '" />';
		echo '<input class="dm_timepicker text_time" type="text" name="', $field['id'], '[time]" id="', $field['id'], '_time" value="', '' !== $meta ? date( 'h:i A', $meta ) : $field['std'], '" />';
	...
	public static function text_time( $field, $meta ) {
		echo '<input class="dm_timepicker text_time" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function select_timezone( $field, $meta ) {
		$meta = '' !== $meta ? $meta : $field['std'];
		if ( '' === $meta )
			$meta = dm_fields::timezone_string();
	...
	public static function text_money( $field, $meta ) {
		echo ! empty( $field['before'] ) ? '' : '$', ' <input class="dm_text_money" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function text_money_repeat( $field, $meta ) {
		self::repeat_text_field( $field, $meta, 'dm_text_money' );
	}
	...
	public static function colorpicker( $field, $meta ) {
		...
		elseif ( ! preg_match( '/^#' . $hex_color . '/i', $meta ) ) // Value doesn't match #123abc, so sanitize to just #.
			$meta = "#";
		echo '<input class="dm_colorpicker dm_text_small" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', $meta, '" />', self::desc( $field['desc'] );
	}
	...
	public static function textarea_code( $field, $meta ) {
		echo '<textarea name="', $field['id'], '" id="', $field['id'], '" cols="60" rows="10" class="dm_textarea_code">', '' !== $meta ? $meta : $field['std'], '</textarea>', self::desc( $field['desc'], true );
	}
	...
	public static function radio_inline( $field, $meta ) {
		if( empty( $meta ) && !empty( $field['std'] ) ) $meta = $field['std'];
		echo '<ul class="dm_radio_inline">';
		$i = 1;
		foreach ($field['options'] as $option) {
			echo '<li class="dm_radio_inline_option"><input type="radio" name="', $field['id'], '" id="', $field['id'], $i, '" value="', $option['value'], '"', checked( $meta == $option['value'] ), ' /> <label for="', $field['id'], $i, '">', $option['name'], '</label></li>';
			$i++;
		}
	...
	public static function title( $field, $meta, $object_id, $object_type ) {
		...
		echo '<'. $tag .' class="dm_metabox_title">', $field['name'], '</'. $tag .'>', self::desc( $field['desc'], true );
	}
	...
	public static function taxonomy_radio( $field, $meta, $object_id ) {
		...
		foreach ( $terms as $term ) {
			...
			echo '<li class="dm_radio_inline_option"><input type="radio" name="', $field['id'], '" id="', $field['id'], $i,'" value="'. $term->slug . '" ', checked( $checked ), ' /> <label for="', $field['id'], $i, '">' . $term->name . '</label></li>';
			$i++;
		}
		echo '</ul>', self::desc( $field['desc'], true );
	}
	...
	public static function file_list( $field, $meta, $object_id ) {
		...
		echo '<input class="dm_upload_file dm_upload_list" type="hidden" size="45" id="', $field['id'], '" name="', $field['id'], '" value="', $meta, '" />';
		echo '<input class="dm_upload_button button dm_upload_list" type="button" value="'. __( 'Add or Upload File', 'dm' ) .'" />', self::desc( $field['desc'], true );

		echo '<ul id="', $field['id'], '_status" class="dm_media_status attach_list">';
			...
			foreach ( $meta as $id => $fullurl ) {
				if ( self::is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p><a href="#" class="dm_remove_file_button">'. __( 'Remove Image', 'dm' ) .'</a></p>
					...
					echo
					'<li>',
						__( 'File:', 'dm' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. __( 'Download', 'dm' ) .'</a> / <a href="#" class="dm_remove_file_button">'. __( 'Remove', 'dm' ) .'</a>)
						<input type="hidden" id="filelist-', $id ,'" name="', $field['id'] ,'[', $id ,']" value="', $fullurl ,'" />
					</li>';
				}
			}
	...
	public static function file( $field, $meta, $object_id, $object_type ) {
		...
		if ( 'url' == $field['allow'] || ( is_array( $field['allow'] ) && in_array( 'url', $field['allow'] ) ) )
			$input_type_url = 'text';
		echo '<input class="dm_upload_file" type="' . $input_type_url . '" size="45" id="', $field['id'], '" name="', $field['id'], '" value="', $meta, '" />';
		echo '<input class="dm_upload_button button" type="button" value="'. __( 'Add or Upload File', 'dm' ) .'" />';
		...
		if ( $meta && ! $_id_meta ) {
			$_id_meta = dm_fields::image_id_from_url( esc_url_raw( $meta ) );
		}

		echo '<input class="dm_upload_file_id" type="hidden" id="', $_id_name, '" name="', $_id_name, '" value="', $_id_meta, '" />',
		self::desc( $field['desc'], true ),
		'<div id="', $field['id'], '_status" class="dm_media_status">';
			if ( ! empty( $meta ) ) {

				if ( self::is_valid_img_ext( $meta ) ) {
					echo '<div class="img_status">';
					echo '<img style="max-width: 350px; width: 100%; height: auto;" src="', $meta, '" alt="" />';
					echo '<p><a href="#" class="dm_remove_file_button" rel="', $field['id'], '">'. __( 'Remove Image', 'dm' ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo __( 'File:', 'dm' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta, '" target="_blank" rel="external">'. __( 'Download', 'dm' ) .'</a> / <a href="#" class="dm_remove_file_button" rel="', $field['id'], '">'. __( 'Remove', 'dm' ) .'</a>)';
				}
			}
		echo '</div>';
	}
	...
	public static function oembed( $field, $meta, $object_id, $object_type ) {
		echo '<input class="dm_oembed" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'], true );
		echo '<p class="dm-spinner spinner" style="display:none;"><img src="'. admin_url( '/images/wpspin_light.gif' ) .'" alt="spinner"/></p>';
		echo '<div id="', $field['id'], '_status" class="dm_media_status ui-helper-clearfix embed_wrap">';

			if ( $meta != '' )
				echo dm_fields_ajax::get_oembed( $meta, $object_id, array(
	...
	/**
	 * Default fallback. Allows rendering fields via "dm_render_$name" hook
	 * @since  1.0.0
	...
	public function __call( $name, $arguments ) {
		...
		do_action( "dm_render_$name", $field, $meta, $object_id, $object_type );
	}
