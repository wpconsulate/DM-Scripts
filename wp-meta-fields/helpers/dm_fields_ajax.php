...
class dm_fields_ajax {
	...
	public function oembed_handler() {
		...
		// verify our nonce
		if ( ! ( isset( $_REQUEST['dm_ajax_nonce'], $_REQUEST['oembed_url'] ) && wp_verify_nonce( $_REQUEST['dm_ajax_nonce'], 'ajax_nonce' ) ) )
			die();
	...
	public function get_oembed( $url, $object_id, $args = array() ) {
		...
		if ( isset( $args['object_type'] ) && $args['object_type'] != 'post' ) {
			...
			add_filter( 'get_post_metadata', array( 'dm_fields_ajax', 'hijack_oembed_cache_get' ), 10, 3 );
			// Sets ombed cache in our object's meta (vs postmeta)
			add_filter( 'update_post_metadata', array( 'dm_fields_ajax', 'hijack_oembed_cache_set' ), 10, 4 );
		...
		if ( $check_embed && $check_embed != $fallback )
			return '<div class="embed_status">'. $check_embed .'<p><a href="#" class="dm_remove_file_button" rel="'. $args['field_id'] .'">'. __( 'Remove Embed', 'cmb' ) .'</a></p></div>';
