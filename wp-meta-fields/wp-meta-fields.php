...
spl_autoload_register('dm_fields::autoload_helpers');
...
$meta_boxes = array();
$meta_boxes = apply_filters( 'dm_fields' , $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new dm_fields( $meta_box );
}
...
function get_dm_fields_url() {
	...
	return trailingslashit( apply_filters('dm_fields_url', $dm_url ) );
}

define( 'dm_fields_URL', get_dm_fields_url() );
...
class dm_fields {
	...
	function __construct( $meta_box ) {
		...
		$allow_frontend = apply_filters( 'dm_allow_frontend', true, $meta_box );
		...
		$show_filters = 'dm_fields_Show_Filters';
		foreach ( get_class_methods( $show_filters ) as $filter ) {
			add_filter( 'dm_show_on', array( $show_filters, $filter ), 10, 2 );
		}

		// register our scripts and styles for dm
	...
	/**
	 * Registers scripts and styles for DM
	 * @since  1.0.0
	 */
	function register_scripts() {
		...
		// scripts required for dm
		$scripts = array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', /*'media-upload', */'dm-timepicker' );
		// styles required for dm
		$styles = array();
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
			   ...
			   wp_register_script( 'iris', admin_url( 'js/iris.min.js' ), array( 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ), self::dm_VERSION );
		   	   wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::dm_VERSION );
		...
		wp_register_script( 'dm-timepicker', dm_fields_URL . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'dm-scripts', dm_fields_URL .'js/dm'. $min .'.js', $scripts, self::dm_VERSION );

		wp_enqueue_media();

		wp_localize_script( 'dm-scripts', 'dm_l10', array(
		...
		wp_register_style( 'dm-styles', dm_fields_URL . 'style'. $min .'.css', $styles );
	...
	/**
	 * Enqueues scripts and styles for DM
	 * @since  1.0.0
	 */
	function do_scripts( $hook ) {
		...
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			...
			// default is to show dm styles on post pages
			if ( ! isset( $this->_meta_box['dm_styles'] ) || $this->_meta_box['dm_styles'] != false )
	...
	function add_metaboxes() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if ( apply_filters( 'dm_show_on', true, $this->_meta_box ) )
	...
	function user_metabox() {
		...
		// default is to NOT show dm styles on user profile page
		if ( isset( $this->_meta_box['dm_styles'] ) && $this->_meta_box['dm_styles'] == true  )
			wp_enqueue_style( 'dm-styles' );
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		// get box types
		$types = dm_fields_types::get();

		// Use nonce for verification
		echo "\n<!-- Begin DM Fields -->\n";
		wp_nonce_field( self::nonce(), 'wp_meta_box_nonce', false, true );
		do_action( 'dm_before_table', $meta_box, $object_id, $object_type );
		echo '<table class="form-table dm_metabox">';
		...
		foreach ( $meta_box['fields'] as $field ) {
			...
			$field['std'] = apply_filters( 'dm_std_filter', $field['std'], $field, $object_id, $object_type );
			...
			// (assuming no one would want to save 'dm_override_val' as a value)
			$meta = apply_filters( 'dm_override_meta_value', 'dm_override_val', $object_id, $field, $object_type );

			// If no override, get our meta
			if ( $meta === 'dm_override_val' )
			...
			$repeatclass = $repeat ? ' dm-repeat' : '';
			$repeatmethod = $repeat ? '_repeat' : '';

			echo '<tr class="dm-type-'. sanitize_html_class( $field['type'] ) .' dm_id_'. sanitize_html_class( $field['id'] ) . $repeatclass .'">';
		...
		echo '</table>';
		do_action( 'dm_after_table', $meta_box, $object_id, $object_type );
		echo "\n<!-- End DM Fields -->\n";
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		if ( ! apply_filters( 'dm_show_on', true, $meta_box ) )
			return;
		...
		foreach ( $meta_box['fields'] as $field ) {
			...
			$new = apply_filters( 'dm_validate_'. $field['type'], $new, $object_id, $field, $object_type );

			// Allow validation via metabox flag
			if ( isset( $field['validate_func'] ) ) {
				$ok = call_user_func( array( 'dm_fields_Validate', $field['validate_func'] ), $new );
		...
		do_action( "dm_save_{$object_type}_fields", $object_id, $meta_box['id'], $updated, $meta_box );
...
// Handle oembed Ajax
add_action( 'wp_ajax_dm_oembed_handler', array( 'dm_fields_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_dm_oembed_handler', array( 'dm_fields_ajax', 'oembed_handler' ) );
 ...
 * @param int   $object_id  Object ID
 */
function dm_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		dm_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function dm_print_metabox( $meta_box, $object_id ) {
    
	$dm = new dm_fields( $meta_box );
	if ( $dm ) {

		dm_fields::set_object_id( $object_id );

		if ( ! wp_script_is( 'dm-scripts', 'registered' ) )
			$dm->register_scripts();

		wp_enqueue_script( 'dm-scripts' );

		// default is to show dm styles
		if ( ! isset( $meta_box['dm_styles'] ) || $meta_box['dm_styles'] != false )
			wp_enqueue_style( 'dm-styles' );

		dm_fields::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function dm_save_metabox_fields( $meta_box, $object_id ) {
	dm_fields::save_fields( $meta_box, $object_id );
}
 ...
 * @return string             DM html form markup
 */
function dm_metabox_form( $meta_box, $object_id, $echo = true ) {

	// Make sure that our object type is explicitly set by the metabox config
	dm_fields::set_object_type( dm_fields::set_mb_type( $meta_box ) );
	...
	if (
		// check nonce
		isset( $_POST['submit-dm'], $_POST['object_id'], $_POST['wp_meta_box_nonce'] )
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], dm_fields::nonce() )
		&& $_POST['object_id'] == $object_id
	)
	
    dm_save_metabox_fields( $meta_box, $object_id );
	...
	$form_format = apply_filters( 'dm_frontend_form_format', '<form class="dm-form" method="post" id="%s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%s">%s<input type="submit" name="submit-dm" value="%s"></form>', $object_id, $meta_box, $form );

	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'dm' ) );
