...
class WPMU_Network_Publish_Base {
...
**/			
	...
	/**
		Creates a new DM_Form.
		
		@param		$options	Default options to send to the DM form constructor.
		@return					A new DM form class.
	**/ 
	public function form($options = array())
	{
		$options = array_merge($options, array('language' => preg_replace('/_.*/', '', get_locale())) );
		if (class_exists('DM_Form'))
			return new DM_Form($options);
		require_once('DM_Form.php');
		return new DM_Form($options);
	}
