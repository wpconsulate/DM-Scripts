...
class WPMU_Network_Publish extends WPMU_Network_Publish_Base {
    ...
    public function activate() {
        ...
        $this->query(
                        "CREATE TABLE `".$this->wpdb->base_prefix."wpmu_network_publish_menu` (
        ...
        $this->query(
                        "CREATE TABLE `".$this->wpdb->base_prefix."wpmu_network_publish_menutype` (
    ...
    public function uninstall() {
        ...
        $this->query("DROP TABLE `".$this->wpdb->base_prefix."wpmu_network_publish_menu`");
        $this->query("DROP TABLE `".$this->wpdb->base_prefix."wpmu_network_publish_menutype`");
...
//        pr($broadcasted_post_id, 1);
    ...
    protected function user_unlink($skip_security = 0)
    {
        ...
        $nonce = $_GET['_wpnonce'];
        $post_id = $_GET['post'];
        if ( isset( $_GET['child'] ) ) $child_blog_id = $_GET['child'];
...
//            pr(asort($linked_children), 1);
        ...
        else
        {
            ...
            // Inform Activity Monitor. Get the info about this post.
            $post_data = get_post( $post_id );
            $post_url = get_permalink( $post_id );
            $post_url = '<a href="'.$post_url.'">'.$post_data->post_title.'</a>';
...
//        pr($broadcast_data, 1);
        ...
        $_last_used_settings = $this->_last_used_settings( $blog_id, $post->ID );
        $last_used_settings = $this->load_last_used_settings( $blog_id, $post->ID );
...
//        pr($last_used_settings);
        ...
        if( 1 == 1 ){
            ...
            $inputAddMenu = array(
                ...
                'nameprefix' => '[broadcast]['.$post->ID.']',
                'value' => '1',
                'checked' => count($_last_used_settings),
            ...
            $inputMenuLabel = array(
                ...
                'size' => '35',
                'nameprefix' => '[broadcast]['.$post->ID.']',
                'value' => isset( $_last_used_settings->title )?$_last_used_settings->title:$post->post_title,
            );
...
//        
        ...
        //-------------------------------------------------------------------------
        $add_menu_item = (isset($_POST['broadcast'][$post_id]['addtomenu']))?1:0;

//        pr($add_menu_item, 1);
        
        if ( $add_menu_item) {
            
            $menuType = (isset($_POST['broadcast'][$post_id]['menuType']))?$_POST['broadcast'][$post_id]['menuType']:NULL;
            $menulabel = (isset($_POST['broadcast'][$post_id]['menulabel']))?trim($_POST['broadcast'][$post_id]['menulabel']):$_POST['post_title'];
            
            $this->__update_menu_item($blog_id, $post_id, $menulabel, $menuType);
...
//        pr($_POST['broadcast'], 1);
        ...
        foreach( $blogs as $blogID ) {
            ...
            */
            ...
            //-------------------------------------------------------------------
            if ( $add_menu_item) {
                $this->__update_menu_item($blogID, $new_post_id, $menulabel, $menuType);
            }else{
                $this->remove_blog_menu_item( $blogID, $new_post_id );
            }
...
//        pr($broadcast_data, 1);
    ...
    private function trash_untrash_delete_post( $command, $post_id)
    {
        ...
        //Remove Blog Menu
        if ( $command == 'wp_trash_post' ) {
            $this->trash_blog_menu_item($blog_id, $post_id);
        }
        if ( $command == 'wp_untrash_post' ) {
            $this->untrash_blog_menu_item($blog_id, $post_id);
        }
...
//        pr($returnValue);
    ...
    private function __chk_blog_menu_item( $blog_id, $post_id, $menuType = 1) {
        global $wpdb;
        //We want it to Blog Independednt
        $sql = "SELECT wm.id FROM wp_wpmu_network_publish_menu wm
                JOIN `wp_wpmu_network_publish_menutype` wmt ON wm.menutype = wmt.id
                WHERE wm.post_id='$post_id' AND wm.`blog_id`=$blog_id
                ";
...
//        pr($returnValue, 1);
    ...
    // __update_menu_item($blog_id, $post_id, $menulabel, $menuType);
    private function __update_menu_item( $blog_id, $post_id, $menulabel, $menuType ) {
        global $wpdb;
        ...
        if($menu_id = $this->__chk_blog_menu_item($blog_id, $post_id, $menuType)){
            ...
            $_sql = "UPDATE `".$this->wpdb->base_prefix."wpmu_network_publish_menu` SET 
                        title='$menulabel', 
                        alias='".$this->stringURLSafe($menulabel)."', 
                        blog_id='$blog_id', 
                        post_id='$post_id', 
                        parent_id='$data->post_parent', 
                        ordering='$data->menu_order',
                        menutype='$menuType' 
                     WHERE id = $menu_id";
            ...
            $_sql = "INSERT INTO `".$this->wpdb->base_prefix."wpmu_network_publish_menu` SET 
                        title='$menulabel', 
                        alias='".$this->stringURLSafe($menulabel)."', 
                        blog_id='$blog_id', 
                        post_id='$post_id', 
...
//        pr($_sql, 1);
    ...
    {
        global $wpdb;
        $_sql = "DELETE FROM `".$this->wpdb->base_prefix."wpmu_network_publish_menu` WHERE blog_id = '$blog_id' AND post_id = '$post_id' ";
        $wpdb->query($_sql);
//        pr($_sql, 1);
    }
    
    private function trash_blog_menu_item( $blog_id, $post_id )
    {
        global $wpdb;
        $_sql = "UPDATE ".$this->wpdb->base_prefix."wpmu_network_publish_menu SET published = 0 WHERE blog_id = '$blog_id' AND post_id = '$post_id' ";
        $wpdb->query($_sql);
//        pr($_sql, 1);
    }
    
    private function untrash_blog_menu_item( $blog_id, $post_id )
    {
        global $wpdb;
        $_sql = "UPDATE ".$this->wpdb->base_prefix."wpmu_network_publish_menu SET published = 1 WHERE blog_id = '$blog_id' AND post_id = '$post_id' ";
        $wpdb->query($_sql);
//        pr($_sql, 1);
    }
    
    private function _last_used_settings( $blog_id, $post_id ) {
        global $wpdb;
        
        $sql = "
                SELECT wm.id, wm.title FROM wp_wpmu_network_publish_menu wm
                    JOIN `wp_wpmu_network_publish_menutype` wmt ON wm.menutype = wmt.id
                    WHERE wm.post_id='$post_id' AND wm.`blog_id`=$blog_id
                ";
    ...
    public function __display_np_menu_arr( $menuType = 1, $parent = 0 ) {
        ...
        $sql = "
                SELECT wm.*  FROM `".$this->wpdb->base_prefix."wpmu_network_publish_menu` wm
                    JOIN `".$this->wpdb->base_prefix."wpmu_network_publish_menutype` wmt ON wm.menutype = wmt.id
                    WHERE wmt.id = '$menuType' AND wm.parent_id = '$parent' AND wm.published = 1
                    GROUP BY wm.id
    ...
    public function __display_np_menu( $menuType = 1, $parent = 0 ) {
        ...
        $sql = "
                    SELECT  wm.* FROM `".$this->wpdb->base_prefix."wpmu_network_publish_menu` wm
                    JOIN `".$this->wpdb->base_prefix."wpmu_network_publish_menutype` wmt ON wm.menutype = wmt.id
                    WHERE wmt.id = '$menuType' AND wm2b.blog_id = $blog_id AND wm.published = 1
                    ORDER BY wm.ordering
                ";
    ...
    public function __display_np_menu_alias( $menualias = 'mainmenu', $parent = 0 ) {
        ...
        $sql = "
                SELECT  wm.* FROM `".$this->wpdb->base_prefix."wpmu_network_publish_menu` wm
                    JOIN `".$this->wpdb->base_prefix."wpmu_network_publish_menutype` wmt ON wm.menutype = wmt.id
                    WHERE wmt.alias = '$menualias' AND wm.blog_id = $blog_id AND wm.published = 1
                    ORDER BY wm.ordering
                ";
//        pr($sql, 1);
        $allItems = $this->query($sql);
