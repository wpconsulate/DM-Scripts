...
//        pr($returnValue, 1);
    ...
    */
    ...
    private function _last_used_settings( $blog_id, $post_id )
    {
        ...
        $sql = "SELECT * FROM `".$this->wpdb->base_prefix."wmpu_network_publish_menu` m
                JOIN `".$this->wpdb->base_prefix."wpmu_network_publish_menu_to_blog` mb ON m.`id` = mb.`menu_id`
                WHERE mb.`blog_id`=$blog_id AND mb.`post_id`= $post_id";
...
//        pr($_sql, 1);
    ...
    {
        $data = get_post($post_id);
        $_sql = "INSERT INTO `".$this->wpdb->base_prefix."wpmu_network_publish_menu_to_blog` (blog_id, menu_id, post_id, parent_id) VALUES ( '$blog_id',  '$menu_id', '$post_id', '$data->post_parent' )";
//        pr($data, 1);
        $this->query($_sql);
    }
...
//        pr($returnValue, 1);
    ...
    public function __display_np_menu_alias( $menualias = 'mainmenu', $parent = 0 ) {
        ...
        $sql = "
                SELECT  wm.id,wm.title,wm2b.post_id, wm2b.parent_id AS parent, wm.ordering FROM `".$this->wpdb->base_prefix."wmpu_network_publish_menu` wm
                    JOIN `".$this->wpdb->base_prefix."wpmu_network_publish_menu_to_blog` wm2b ON wm.id = wm2b.menu_id
                    JOIN `".$this->wpdb->base_prefix."wmpu_network_publish_menutype` wmt ON wm.menutype = wmt.id
